...
import numpy as np
from app.services.inference.common import (
    ClosableQueue,
    Face,
    Face2Search,
    FaceNew,
...
class Extractor:
    ...
    def __init__(self):
        root: Path = (
            Path(__file__)
            .parents[1]
            .joinpath("model_zoo\\models\\insightface\\irn50_glint360k_r50.onnx")
        )
    ...
    def run_onnx(
        self, img2extract: Image, bbox: Bbox, kps: Kps, det_score: float
    ) -> Embedding:
        """
...
class IdentifyWorker(Process):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            daemon=True,
            args=(task_queue, result_dict, self._stop_event),
        )
...
class IdentifierTask(Tracker):
    ...
    def __init__(
        self, jobs: ClosableQueue, results: ClosableQueue, manager: IdentifyManager
    ):
